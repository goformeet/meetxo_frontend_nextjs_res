...
const FormSchema = z.object({
    ...
    username: z.string().min(4, {
        message: 'User Name must be minimum 4 characters.',
    }),
...
export default function DetailsForm({ handleSubmit, modal = false }: { handleSubmit: (details: { name: string;  email: string; username?: string}) => void; modal?: boolean }) {
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="flex flex-col items-start w-full mt-6">
                                    <FormLabel className="text-left text-sm font-plus-jakarta-sans">User Name <span className="text-[#E03137]">*</span></FormLabel>
                                    <FormControl className="w-full">
                                        <Input className="py-4 px-5 h-11 md:h-14" placeholder="Enter your name" {...field} />
                                    </FormControl>
                                    <FormMessage />
